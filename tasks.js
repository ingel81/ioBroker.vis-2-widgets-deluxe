...
function copyAllFiles() {
    copyFiles(['src-widgets/build/**/*', '!src-widgets/build/index.html'], 'widgets/vis-2-widgets-deluxe/', {
        process: (fileData, fileName) => {
            if (fileName.includes('installSVGRenderer')) {
                ...
                if (match) {
                    ...
                    fileData = fileData.replace(match[0], `${match[1]}=value=>typeof value === "function";${match[0]}`); // prevent error
                }
                return fileData;
            }
        },
